...
class RobotJointController:
    ...
    def __init__(self, target_min=-100, target_max=100, gripper_target_min=0, gripper_target_max=100):
        """
        Initializes the controller and pre-computes arrays for vectorized operations.
        """
        self.target_min = target_min
        ...
        self.gripper_target_min = gripper_target_min
        self.gripper_target_max = gripper_target_max
        self.gripper_target_range = gripper_target_max - gripper_target_min

        # Fixed joint order
        self.joint_names = list(self.JOINT_RANGES_MAP.keys())

        # Ranges for vectorized operations
        ranges = np.array(list(self.JOINT_RANGES_MAP.values()))
        self.min_ranges_rad = ranges[:, 0]
        self.max_ranges_rad = ranges[:, 1]
        self.original_ranges_rad = self.max_ranges_rad - self.min_ranges_rad
    ...
    def _normalize(self, joint_angles: dict[str, float]) -> dict[str, float]:
        ...
        for joint_name, angle in joint_angles.items():
            ...
            if joint_name == 'gripper':
                scaled_value = self.gripper_target_min + (angle - min_orig) * (self.gripper_target_range / original_range)
                normalized_angles[joint_name] = np.clip(scaled_value, self.gripper_target_min, self.gripper_target_max)
            else:
                scaled_value = self.target_min + (angle - min_orig) * (self.target_range / original_range)
                normalized_angles[joint_name] = np.clip(scaled_value, self.target_min, self.target_max)
        return normalized_angles
    ...
    def _unnormalize(self, normalized_angles: dict[str, float]) -> dict[str, float]:
        ...
        for joint_name, norm_value in normalized_angles.items():
            ...
            if joint_name == 'gripper':
                norm_value = np.clip(norm_value, self.gripper_target_min, self.gripper_target_max)
                original_value = min_orig + (norm_value - self.gripper_target_min) * (original_range / self.gripper_target_range)
            else:
                norm_value = np.clip(norm_value, self.target_min, self.target_max)
                original_value = min_orig + (norm_value - self.target_min) * (original_range / self.target_range)
    ...
    def _normalize_array(self, joint_angles_array: np.ndarray) -> np.ndarray:
        ...
        scaled_array = np.empty_like(joint_angles_array)

        for idx, joint_name in enumerate(self.joint_names):
            min_orig = self.min_ranges_rad[idx]
            range_orig = self.original_ranges_rad[idx]
            angle = joint_angles_array[idx]

            if joint_name == 'gripper':
                scaled = self.gripper_target_min + (angle - min_orig) * (self.gripper_target_range / range_orig)
                scaled_array[idx] = np.clip(scaled, self.gripper_target_min, self.gripper_target_max)
            else:
                scaled = self.target_min + (angle - min_orig) * (self.target_range / range_orig)
                scaled_array[idx] = np.clip(scaled, self.target_min, self.target_max)
    ...
    def _unnormalize_array(self, normalized_angles_array: np.ndarray) -> np.ndarray:
        ...
        unnormalized_array = np.empty_like(normalized_angles_array)

        for idx, joint_name in enumerate(self.joint_names):
            min_orig = self.min_ranges_rad[idx]
            range_orig = self.original_ranges_rad[idx]
            norm_value = normalized_angles_array[idx]

            if joint_name == 'gripper':
                norm_value = np.clip(norm_value, self.gripper_target_min, self.gripper_target_max)
                original = min_orig + (norm_value - self.gripper_target_min) * (range_orig / self.gripper_target_range)
            else:
                norm_value = np.clip(norm_value, self.target_min, self.target_max)
                original = min_orig + (norm_value - self.target_min) * (range_orig / self.target_range)

            unnormalized_array[idx] = original
...
if __name__ == "__main__":
    ...
    home_position_array = np.array([0.0, -1.75, 0.0, 0.0, 0.0, 1.74532])  # [rad] -0.17453, 1.74532
    normalized_array = controller._normalize_array(home_position_array)
    print("Original Array (rad):", np.round(home_position_array, 4))
    print("Normalized Array ([-100,100] or [0,100]):", np.round(normalized_array, 2))

    print("\n" + "="*40 + "\n")

    unnormalized_array = controller._unnormalize_array(normalized_array)
    print("Reconstructed Array (rad):", np.round(unnormalized_array, 4))
