...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        sent_action = robot_to_control.send_action(action)
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    if cfg.enable_sim:
        # If simulation is enabled, we always create a simulated robot.
        # The config for the simulated robot is derived from the real robot config,
        # but its type is changed to so101_sim_follower.
        sim_robot_config = SO101SimFollowerConfig(
            id=cfg.robot.id,
            cameras=cfg.robot.cameras,
            port=cfg.robot.port, # Port might still be relevant for some sim setups or if it's a hybrid.
            ...
            # mujoco_model_path is set by default in SO101SimFollowerConfig
        )
        sim_robot = make_robot_from_config(sim_robot_config)

        if not cfg.disable_follower:
            # If real follower is enabled, create a real robot instance.
            real_robot = make_robot_from_config(cfg.robot)
            robot_to_control = real_robot
            robot_for_obs = real_robot
        else:
            # If real follower is disabled, only use the simulated robot.
            robot_to_control = sim_robot
            robot_for_obs = sim_robot
    else:
        # If simulation is not enabled, use the real robot as usual.
        real_robot = make_robot_from_config(cfg.robot)
        robot_to_control = real_robot
        robot_for_obs = real_robot
    ...
    action_features = hw_to_dataset_features(robot_for_obs.action_features, "action", cfg.dataset.video)
    obs_features = hw_to_dataset_features(robot_for_obs.observation_features, "observation", cfg.dataset.video)
    ...
    if cfg.resume:
        ...
        if hasattr(robot_for_obs, "cameras") and len(robot_for_obs.cameras) > 0:
            dataset.start_image_writer(
                num_processes=cfg.dataset.num_image_writer_processes,
                num_threads=cfg.dataset.num_image_writer_threads_per_camera * len(robot_for_obs.cameras),
            )
        sanity_check_dataset_robot_compatibility(dataset, robot_for_obs, cfg.dataset.fps, dataset_features)
    else:
        ...
        dataset = LeRobotDataset.create(
            ...
            cfg.dataset.fps,
            root=cfg.dataset.root,
            robot_type=robot_for_obs.name,
            features=dataset_features,
            use_videos=cfg.dataset.video,
            image_writer_processes=cfg.dataset.num_image_writer_processes,
            image_writer_threads=cfg.dataset.num_image_writer_threads_per_camera * len(robot_for_obs.cameras),
        )
    ...
    while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
        ...
        record_loop(
            robot=robot_for_obs, # Observations always from robot_for_obs
            events=events,
            fps=cfg.dataset.fps,
            teleop=teleop,
            policy=policy,
            ...
            robot_to_control=robot_to_control, # Pass the robot to control
            sim_robot=sim_robot, # Pass sim_robot for mirroring actions
            real_robot=real_robot, # Pass real_robot for mirroring actions
